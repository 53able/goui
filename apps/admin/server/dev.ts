...
const startDevServer = async () => {
  ...
  await createVite();
  console.log('✅ Vite dev server ready at http://localhost:5174');
  ...
  app.use('*', async (c, next) => {
    ...
    // Viteアセットへのリクエストは認証をスキップ
    if (
      c.req.path.startsWith('/src/') ||
      c.req.path.startsWith('/@') ||
      c.req.path.startsWith('/node_modules/')
    ) {
      return next();
    }
  ...
  for (const path of VITE_PROXY_PATHS) {
    app.use(path, async (c) => {
      ...
      const viteUrl = `http://localhost:5174${url.pathname}${url.search}`;
      try {
  ...
  console.log('');
  console.log('🔧 Admin Development Server (Vite SSR Integration)');
